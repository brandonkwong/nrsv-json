...
function structureSanity (bibleData) {
  ...
  const structureCanon = (canon) => {
    const data = {
      ...
      _type: 'canon',
      name: canon.name,
      slug: slugify(canon.abbreviation),
      books: []
    };
  ...
  const structureBook = (book) => {
    const data = {
      ...
      _type: 'book',
      name: book.name,
      slug: slugify(book.abbreviation),
      chapters: book.chapters,
      canon: {
        _type: 'reference',
        _ref: canons.find(canon => canon.slug === slugify(book.canon))?._id
      }
    };
  ...
  const structureChapter = (chapter, bookId) => {
    const data = {
  ...
  const structureVerse = (verse, bookId, chapterId) => {
    const data = {
  ...
  for (let i = 0; i < books.length; ++i) {
    ...
    for (let j = 0; j < book.chapters.length; ++j) {
      const chapter = structureChapter(book.chapters[j], book._id);
      books[i].chapters[j] = chapter;

      for (let k = 0; k < chapter.verses.length; ++k) {
        const verse = structureVerse(chapter.verses[k], book._id, chapter._id);
